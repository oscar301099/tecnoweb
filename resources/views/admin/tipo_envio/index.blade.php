                ...
                <tbody>
                    @foreach($tipos as $tipo)
                        <tr>
                            ...
                            <td width="10px">
                                <a class="btn btn-outline-primary" href="{{route('admin.tipo_envios.edit', $tipo)}}">
                                    <i class="material-icons fa fa-pen"></i>
                                </a>
                            </td>

                            <td width="10px">
                                <form action="{{ route('admin.tipo_envios.destroy', $tipo->id )}}" method="POST" onsubmit="return confirm('¿Estas seguro de eliminar: {{$tipo->nombre}}?')">
                                    @csrf
                                    @method('DELETE')
